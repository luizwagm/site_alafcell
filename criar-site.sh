#!/usr/bin/env bash
# ==========================================================================
# ALAFCELL ASSISTEC — criar o site no servidor
#
#   sudo ./criar-site.sh                        → alafcell.projetos.luizaugust.me
#   sudo ./criar-site.sh alafcell.com.br        → o domínio de verdade
...
#
# 1. NÃO EXISTE www NUM SUBDOMÍNIO. `www.alafcell.projetos.luizaugust.me` não é
...
DOMINIO="${1:-alafcell.projetos.luizaugust.me}"
PORTA="${2:-5202}"
...
if [ "$CODIGO" != "200" ]; then
  ...
  vermelho "     sudo systemctl status alafcell"
  exit 1
fi
...
if grep -q '^ALAFCELL_SITE=' "$ENV" 2>/dev/null; then
  sed -i "s|^ALAFCELL_SITE=.*|ALAFCELL_SITE=https://$DOMINIO|" "$ENV"
else
  echo "ALAFCELL_SITE=https://$DOMINIO" >> "$ENV"
fi
chown deploy:deploy "$ENV" 2>/dev/null || true
verde "     ALAFCELL_SITE=https://$DOMINIO"
[ "$SUBDOMINIO" -eq 1 ] && amarelo "     endereço de trabalho: o site vai sair FORA do índice do Google"

# No domínio de VERDADE o conteúdo de demonstração não pode continuar no ar:
# preço de referência e chave Pix que não recebe dinheiro deixam de ser
# material de apresentação e passam a ser promessa falsa ao cliente final.
if [ "$SUBDOMINIO" -eq 0 ]; then
  echo
...
server {
    ...
    # ------------------------------------------------------------------
    # ESTÁTICOS. As 29 fotos do acervo pesam 2,8 MB no total: sem cache, o
    ...
    # Freio de borda dos formulários e da consulta de ordem: o pedido nem
    # acorda o processo. A aplicação tem travas próprias — esta é a de fora.
    location ~ ^/(contato|busca-e-leva|acompanhar|checkout/enviar)/?\$ {
        limit_req zone=alafcell_forms burst=5 nodelay;
        proxy_pass http://127.0.0.1:$PORTA;
        include /etc/nginx/proxy_alafcell.conf;
    }
...
cat > /etc/nginx/proxy_alafcell.conf <<'PROXY'
...
# O ALAFCELL_SITE é lido na subida: sem reiniciar, o canonical continua o antigo.
systemctl restart alafcell.service
sleep 2
