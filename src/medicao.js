...
/* ==========================================================================
   ...
   clique em "Aceitar", e "Recusar" é botão de verdade, do mesmo tamanho.
   ...
   nunca fica cego, porque nunca dependeu do Google para saber quantas pessoas
   entraram.
   ========================================================================== */
const { txt } = require("./db");

/* Um ID que não é ID quebraria a página com erro de script. Os dois formatos
   são conhecidos e a conferência é barata. Campo em branco não gera nada. */
...
/* ==========================================================================
   ...
   O carregador fica no HTML de toda página, mas não carrega nada por conta
...
function cabeca() {
  ...
  return `
<script>
/* Medição da Alafcell — só entra depois do aceite. Ver src/medicao.js. */
(function () {
  ...
  window.alafcellMedir = function () { ligarGA4(); ligarPixel(); };

  /* Consentimento vale seis meses. Prazo indefinido não existe na LGPD, e seis
     meses é o que o mercado pratica sem incomodar quem já respondeu. */
  try {
    var g = /(?:^|;\\s*)alafcell_medir=([^;]*)/.exec(document.cookie);
    if (g && g[1] === "sim") window.alafcellMedir();
  } catch (e) {}
})();
</script>`;
}
...
<div class="consent" id="consent" hidden>
  <p><strong>A gente mede o que funciona.</strong>
    Usamos Google Analytics e Meta Pixel para saber quais consertos e aparelhos
    são mais procurados. Nenhum dado seu é vendido, e o site funciona igual se
    você recusar — a nossa contagem de visitas não depende disso.</p>
...
<script>
(function () {
  var cx = document.getElementById("consent");
  if (!cx) return;
  if (/(?:^|;\\s*)alafcell_medir=/.test(document.cookie)) return;   /* já respondeu */
  cx.hidden = false;
  cx.addEventListener("click", function (e) {
    var b = e.target.closest("[data-medir]"); if (!b) return;
    var v = b.dataset.medir;
    document.cookie = "alafcell_medir=" + v + "; Path=/; Max-Age=" + (180 * 24 * 3600) + "; SameSite=Lax";
    cx.hidden = true;
    if (v === "sim" && window.alafcellMedir) window.alafcellMedir();
  });
})();
</script>`;
}
